记录决策是重要的，所以才会存在这个文档。但是保证决策被正确执行更加重要。
...
>
> 1.如果Statefulset描述了存储的信息，则创建对应的PVC信息(如果PVC已经存在，则不做任何操作)
>
> 2.如果Statefulset只是引用ConfigMap或Secret的名称，那么不创建任何资源，因为可能通过上层的工作流构建来实现。
>
> 3.如果

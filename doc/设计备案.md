...
临时存储，对于定义了 `emptyDir` 卷的 Pod，在 Pod 被指派到某节点时此卷会被创建。 就像其名称所表示的那样，`emptyDir` 卷最初是空的。尽管 Pod 中的容器挂载 `emptyDir` 卷的路径可能相同也可能不同，但这些容器都可以读写 `emptyDir` 卷中相同的文件。 当 Pod 因为某些原因被从节点上删除时，`emptyDir` 卷中的数据也会被永久删除。
...
配置文件，在Kubernetes中有四种方式来使用ConfigMap配置Pod中的容器：

1. 在容器命令和参数内
2. 容器的环境变量
3. 在只读卷里面添加一个文件，让应用来读取
4. 编写代码在Pod中运行，使用Kubernetes API来读取ConfigMap(必须编写代码才能读取ConfigMap和它的数据,目前平台不做干预)



在KubeMin-Cli中针对这三种配置，使用不同的数据结构来实现：

1.

2.在

3.在Storage存储特性中添加一个ConfigMap，那么默认将其作为一个文件使用
...
"traits": {
  "storage": [
          {
            "name": "foo",//自定义挂载名
            "sourceName":"myconfigmap",//引用ConfigMap的名称
            "type":"config",
            "mountPath":"/etc/foo",
            "readOnly":true //默认只读
          }
...
使用特性中的Env可以将外部的CofigMap或Secret作为环境变量让Pod使用。上述Yaml对应到特征中的Json为：

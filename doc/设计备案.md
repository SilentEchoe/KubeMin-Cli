...
1.DB作为队列/租约：用 MySQL 做任务“租约+心跳”，所有实例都可竞争任务

2.Leader 负责调度/入队（如 Redis Streams/Kafka），多个执行者消费。

> KubeMin-Cli天然基于云原生，所以我想使用k8s Lease做租约和心跳。



按照我的思路，我将分布式工作流分成几个步骤：

1.服务启动时，使用k8s Lease获取对应的副本数量，当副本数量大于等于3时，为分布式模式：

1.1 分布式模式下，获取领导的节点服务获得读取Mysql数据的权限(现阶段每三秒扫描一次数据库)，并且拥有发送任务的权利

1.2 分布式模式下，会先判断副本数量，通过副本数量来决定领导者服务的工作内容，如果副本数为1，则领导者承担工作流的发送和执行。如果副本书大于等于3，领导者只需要扫扫描+发送

1.3 分布式的模式由副本数量来决定

2.我希望使用k8s Lease做租约和心跳

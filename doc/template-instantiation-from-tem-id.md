  ...
  "component": [
    {
      ...
      "Tem": { "id": "4tbupjg43ln3yj249l0v0fv8", "target": "mysql" }
    }
  ]
}
```
...
2. 查询模板：按 `Tem.id` 读取 `min_applications` 及其组件；校验模板状态可用且 `tmp_enable=true` 时才允许被引用（若为 false，则拒绝作为模板来源）。
3. 克隆模板：
...
- 覆盖规则（单模板多条目不重复克隆）：如果同一个 `Tem.id` 在请求中出现多次，只克隆一套模板组件，后续同模板条目仅用于覆盖匹配的组件（按类型优先匹配），支持：
  ...
  - 精确匹配：推荐在 `Tem` 中提供 `target`（模板组件名），确保覆盖应用到指定组件；缺失 target 时按类型匹配并存在不确定性。
